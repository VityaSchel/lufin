...
describe(`Lufin with ${dbName}+${storageType}`, async () => {
	...
	await test("should throw on invalid upload request", async () => {
		const body = new FormData();
		const req = await fetch("http://backend:3000/upload/" + page.tmpUploadId, {
			method: "POST",
			body,
		});
	...
	await test("should throw on get page request while it's pending", async () => {
		const req = await fetch("http://backend:3000/page/" + page.links.download);
		expect(req.status).toEqual(404);
	});
			...
			.object({
				...
				files: z
					.array(
						z
							.object({
								filename: z.literal("unencrypted.txt"),
								sizeInBytes: z.literal(4),
	...
	await test("should return file content", async () => {
		const req = await fetch(
			"http://backend:3000/page/" + page.links.download + "/unencrypted.txt"
		);
		expect(req.status).toEqual(200);
