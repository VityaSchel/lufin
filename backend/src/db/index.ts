...
try {
	...
	const sqliteFilepath = process.env.SQLITE_DB_FILE_NAME;
	...
	if (
		...
		throw new Error(
			"Please set only one of MONGODB_CONNECTION_STRING or POSTGRESQL_CONNECTION_STRING or SQLITE_DB_FILE_NAME",
		);
	}
	...
	if (mongodbConn) {
		...
		throw new Error(
			"Please set either MONGODB_CONNECTION_STRING or POSTGRESQL_CONNECTION_STRING or SQLITE_DB_FILE_NAME",
		);
	}
} catch (e) {
