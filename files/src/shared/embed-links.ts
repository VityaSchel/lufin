export function getEmbedLinks(pageId: string, file: string) {
  const pageURL = `https://replaceme/${pageId}/${encodeURIComponent(file)}`
  const directURL = `https://replaceme/files/${pageId}/${encodeURIComponent(file)}`
  ...
  return [
    ...
    { name: 'forum_hotlink', text: `[url=https://replaceme][img]${directURL}[/img][/url]` },
    { name: 'website_hotlink', text: `<a href="https://replaceme" target="_blank"><img src="${directURL}" border="0"></img></a>` }
  ]
}