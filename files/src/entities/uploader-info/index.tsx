    ...
    <div className="flex flex-col lg:flex-row gap-16 text-neutral-200 whitespace-pre-wrap [&_p]:leading-[160%] [&_p]:my-2">
      <div className="flex-[4]">
        <Headline variant="h4" className="mb-2">
          {m['terms_footer.expiration']()}
        </Headline>
        <p>{m['upload_form.text']()}</p>
        {/*
        до 500MБ могут храниться до 30 дней,
        файлы размером более 500МБ загрузить нельзя
          */}
        <Headline variant="h4" className="mt-6 mb-2">
          {m['terms_footer.how_it_works.title']()}
        </Headline>
        <p>{m['terms_footer.how_it_works.text']()}</p>
        <p>{m['terms_footer.how_it_works.text2']()}</p>
        <p>{m['terms_footer.how_it_works.text3']()}</p>
        <Headline variant="h4" className="mt-6 mb-2">
          {m['terms_footer.privacy.title']()}
        </Headline>
        <p className="inline">
          {m['terms_footer.privacy.text']().split(' $0 ')[0]}{' '}
          <Link variant="highlighted" href="https://en.wikipedia.org/wiki/Web_Cryptography_API">
            WebCrypto
          </Link>{' '}
          {m['terms_footer.privacy.text']().split(' $0 ')[1]}{' '}
          <Link
            variant="highlighted"
