...
export function UploadableFile({ file, setFile, onRemove, onRename, disableEditing, progress }: {
  ...
  const previewAvailable = previewSupportedMimeTypes.includes(fileType as typeof previewSupportedMimeTypes[number])
      ...
      <Fade in={true}>
        <Collapse orientation='vertical' in={true}>
          <div className={cx(styles.outlined, styles.file)}>
            <div className={styles.mobileTop} onClick={isMobile ? () => setPreviewOpen(!previewOpen) : undefined}>
              ...
              {previewAvailable && isMobile && (
                <span className={styles.openPreview}>{t('uploadable_file.press_to')} {previewOpen ? t('uploadable_file.close') : t('uploadable_file.open')} {t('uploadable_file.preview')}</span>
              )}
            </div>
            {previewAvailable && (
              <div className={cx(styles.preview, { 
                [styles.smallPreview]: ['audio', 'archive'].includes(fileType as typeof previewSupportedMimeTypes[number]),
                [styles.open]: previewOpen 
              })}>
                {fileType === 'image' && file.type !== 'image/gif' && file.type !== 'image/webp' && (
...
const ruLocale = { symbols: { B: 'Б', kB: 'кБ', MB: 'МБ', GB: 'ГБ', TB: 'ТБ', PB: 'ПБ', EB: 'ЭБ', ZB: 'ЗБ', YB: 'ЙБ' } }

function UploadableImageCompressedPreview({ file, setFile }: { file: UploadableFileType, setFile: (file: UploadableFileType) => void }) {
  const { i18n } = useTranslation()
  const originalUrl = React.useMemo(() => URL.createObjectURL((file.isCompressedVersion && file.altBlob) ? file.altBlob : file.blob), [file])
  ...
  const compressedSize = React.useMemo(() => file.isCompressedVersion ? file.blob.size : (file.altBlob?.size ?? null), [file])
  const originalSize = React.useMemo(() => file.isCompressedVersion ? file.altBlob?.size : file.blob.size, [file])
  ...
  return (
    <div className='w-full h-auto flex gap-2'>
      <div className='h-full flex flex-col flex-1 items-center justify-center gap-2 relative'>
        ...
        <img src={originalUrl} className='min-h-0 w-full object-contain'// cursor-pointer' 
          // onClick={() => setIsUsingCompressed(false)} 
        />
        <span className='shrink-0 text-muted'>{originalSize !== undefined && filesize(originalSize, i18n.resolvedLanguage === 'ru' ? ruLocale : undefined)}</span>
      </div>
