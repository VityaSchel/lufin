...
import { AppBar, CircularProgress, Dialog, DialogActions, DialogContent, DialogTitle, IconButton, Toolbar, Tooltip, Typography, useMediaQuery } from '@mui/material'
...
import { FileContentPreview, supportedMimeTypes as previewSupportedMimeTypes } from '$features/file-content-preview'
...
const SharedFile = React.forwardRef(({ pageID, isErrored, downloadProgress, onDownloadStarted, onSave, file, encrypted, content, onOpenPreviousPreview, onOpenNextPreview, i }: {
  pageID: string
  encrypted: boolean
  isErrored: boolean
  ...
  const fileInstance = React.useMemo(() => content && new File([content], file.name, { type: file.mimeType }), [file, content])
  ...
  const handleOpenPreviousPreview = onOpenPreviousPreview && (() => {
    setOpenPreview(false)
    onOpenPreviousPreview()
  })

  const handleOpenNextPreview = onOpenNextPreview && (() => {
    setOpenPreview(false)
    onOpenNextPreview()
  })
  ...
  const handleCopyDirectLink = () => {
    ...
    copy(`${window.location.origin + window.location.pathname}/${encodeURIComponent(file.name)}${window.location.hash}`)
  }
            ...
            (isDownloading && !content)
              ? (
                ...
                <div className={styles.flex}>
                  <Tooltip title={t('direct_link')}>
                    <IconButton onClick={handleCopyDirectLink} onPointerLeave={() => setDirectLinkCopied(false)}>
...
function PreviewButton({ onClick, file, content }: {
  onClick: () => any
  file: SharedFileForDownload
  content?: Blob
}) {
  ...
  const isBlobInNewtabSupported = React.useMemo(() => supportedTypes.includes(
  ...
  return (
    <Tooltip title={t('preview.open_label') + (canOpenInNewTab ? `\n${isMacOS ? '⌘' : 'ctrl'}+${t('click')} — ${t('preview.open_in_new_tab').toLowerCase()}` : '')}>
...
function PreviewDialog({ open, onClose, file, fileInstance, onOpenPreviousPreview, onOpenNextPreview }: {
  open: boolean
  onClose: () => any
  file: SharedFileForDownload
  fileInstance: File
  ...
  useHotkeys([Key.ArrowRight, Key.ArrowLeft], async (_, e) => {
    if (delayedOpen.current !== false && await promiseState(delayedOpen.current) === 'fulfilled') {
      switch(e.keys?.join('')) {
        case 'left':
          onOpenPreviousPreview?.()
          break
  ...
  return (
    ...
    >
          ...
          <AppBar sx={{ position: 'relative' }}>
            <Toolbar>
              ...
              <Typography sx={{ ml: 2, flex: 1, fontSize: '1rem' }} variant="h6" component="div">
                {t('preview.title')} <b>{file.name}</b> 
              </Typography>
            </Toolbar>
          </AppBar>
        ) : (
          <DialogTitle>{t('preview.title')} <b>{file.name}</b></DialogTitle>
        )
      }
      ...
      {!isMobile && (
        <DialogActions>
          <MUIButton onClick={() => onClose()}>{t('ok_button')}</MUIButton>
        </DialogActions>
      )}
    </Dialog>
  )
